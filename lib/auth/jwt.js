...
const attachCookies = ({ res, user, refreshToken }) => {
	const accessTokenJWT = jwt.sign({ payload: { user } }, process.env.JWT_SECRET); // Create an access token
	const refreshTokenJWT = jwt.sign({ payload: { user, refreshToken } }, process.env.JWT_SECRET); // Create a refresh token
