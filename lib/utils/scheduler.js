				...
				for (let i = 0; i < expectedJobLength; i++) {
					if (timeslot !== null) {
						...
						if (scheduledTicket.priorityLevel === "LOW") {
							if (!bumpedJobs.has(scheduledTicket._id.toHexString())) {
								bumpedJobs.add(scheduledTicket._id.toHexString());
							}
						}
					}
				}
...
async function autoSchedule(ticketId) {
	...
	console.log(Object.entries(workerStartTimes[leastWorkedWorker]));
	//TODO consecutive loops error our here vvv
	const chosenDay = Object.entries(workerStartTimes[leastWorkedWorker])[0][0]; // get the day of the chosen time block
	//TODO consecutive loops error our here ^^^
	...
	for (let job of bumpedJobs) {
		autoSchedule(job); // reschedule bumped jobs
	}
